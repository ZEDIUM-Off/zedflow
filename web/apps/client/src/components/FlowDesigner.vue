...
import type { Composition, Kind, RunContext } from '@zedflow/sdk'
...
const props=withDefaults(defineProps<{saved:Composition[];paletteOpen:boolean;context?:RunContext|null;workspaceId?:string;active?:boolean}>(),{active:true})
...
const pairedNode=computed(()=>{const node=selectedNode.value;return node?doc.value.nodes.find(candidate=>candidate.id===(node.data.kind==='context'?requireNodeConfig(node.data.config).modelNode:requireNodeConfig(node.data.config).contextNode)):undefined})
...
const selectedConfig=computed(()=>selectedNode.value?requireNodeConfig(selectedNode.value.data.config):undefined)
const graphEdges=computed({get:()=>doc.value.edges.map(edge=>({...edge,label:edge.label??undefined})),set:(edges:import('@vue-flow/core').Edge[])=>{doc.value.edges=edges.map(({id,source,target,sourceHandle,targetHandle,label})=>({id,source,target,sourceHandle:sourceHandle??null,targetHandle:targetHandle??null,label:typeof label==='string'?label:null}))}})
const predicate=computed<Predicate>({get:()=>selectedConfig.value?.predicate||{kind:'compare',field:selectedConfig.value?.field||'input',operator:'eq',value:selectedConfig.value?.equals},set:value=>{if(selectedNode.value){requireNodeConfig(selectedNode.value.data.config).predicate=value;delete requireNodeConfig(selectedNode.value.data.config).equals;delete requireNodeConfig(selectedNode.value.data.config).field}}})
...
function add(kind:Kind){
  ...
  if(kind==='model'){
    ...
    requireNodeConfig(context.data.config).modelNode=id;config.contextNode=context.id
...
function copyWithPorts(){
  ...
  function convertPorts(graph:Composition):boolean{
    ...
    for(const node of graph.nodes){const nested=requireNodeConfig(node.data.config).composition;if(node.data.kind==='subgraph'&&nested&&!convertPorts(nested))return false}
...
async function connect(connection:Connection){
  ...
  if((doc.value.formatVersion||1)>=4){
    try {
      ...
      const analysis=await client.flows.analyze({...doc.value,edges:[...doc.value.edges,{id,source:connection.source,target:connection.target,sourceHandle:connection.sourceHandle??null,targetHandle:connection.targetHandle??null}]})
  ...
  if((doc.value.formatVersion||1)>=3){
    ...
    if(source.data.kind==='context'){
      ...
      requireNodeConfig(source.data.config).modelNode=target.id;requireNodeConfig(target.data.config).contextNode=source.id
    }
  }
  ...
  doc.value.edges.push({id:crypto.randomUUID(),source:connection.source,target:connection.target,sourceHandle:connection.sourceHandle??null,...((doc.value.formatVersion||1)>=4?{targetHandle:connection.targetHandle??null}:{})})
}
...
function updateConfig(key:string,value:import('@zedflow/sdk').JsonValue){if(selectedNode.value)requireNodeConfig(selectedNode.value.data.config)[key]=value}
function attachChild(id:string){const child=props.saved.find(flow=>flow.id===id);if(selectedNode.value&&child)requireNodeConfig(selectedNode.value.data.config).composition=JSON.parse(JSON.stringify(child))}
</script>
<template>
      <div class="designer">
        ...
        <template v-if="selectedNode.data.kind==='subgraph'"><label>Composition enregistrée<select :value="requireNodeConfig(selectedNode.data.config).composition?.id || ''" @change="attachChild(($event.target as HTMLSelectElement).value)"><option value="" disabled>Choisir une composition</option><option v-for="child in saved.filter(c=>c.id!==doc.id)" :key="child.id" :value="child.id">{{child.name}} · r{{child.revision}}</option></select></label><p class="muted">Entrée input → input. Sortie response → output. Version embarquée et isolée.</p></template><label v-if="selectedNode.data.kind==='set'">Valeur / template<JsonField v-model="requireNodeConfig(selectedNode.data.config).value" label="Valeur JSON ou template" :rows="5"/></label>
        <label v-if="selectedNode.data.kind==='output'">Contenu<textarea v-model="requireNodeConfig(selectedNode.data.config).text" rows="5"/></label>
        <template v-if="selectedNode.data.kind==='condition'"><template v-if="(doc.formatVersion||1)>=2"><PredicateEditor :key="selectedNode.id" v-model="predicate"/><p class="muted">Les valeurs sont comparées sans conversion de type. Utilisez une clé ou un JSON Pointer pour les champs imbriqués.</p></template><template v-else><label>Champ à comparer<input v-model="requireNodeConfig(selectedNode.data.config).field"/></label><JsonField v-model="requireNodeConfig(selectedNode.data.config).equals" label="Valeur attendue JSON" :rows="2"/></template></template>
        <template v-if="['input','inbox'].includes(selectedNode.data.kind)"><label>Question<textarea v-model="requireNodeConfig(selectedNode.data.config).prompt" rows="4"/></label><label>Type de réponse<select v-model="requireNodeConfig(selectedNode.data.config).responseType"><option value="text">Texte libre</option><option value="confirmation">Confirmation</option></select></label></template>
        <NodeSettings :key="selectedNode.id" :node="selectedNode" :composition="doc" :format-version="doc.formatVersion" :workspace-id="workspaceId" @context="emit('context',$event)"/><details v-if="selectedNode.data.kind==='context'&&(doc.formatVersion||1)>=3" class="context-sources"><summary>Sources et capacités accordées</summary><p class="graph-pair-note">Ces ressources sont disponibles pour la stratégie. Son programme décide de leur présence dans la fenêtre.</p><AttachmentEditor :key="selectedNode.id" v-model="requireNodeConfig(selectedNode.data.config).attachments" v-model:slot="attachmentSlot" :context="context"/></details><details class="raw"><summary>Configuration JSON</summary><pre>{{ JSON.stringify(requireNodeConfig(selectedNode.data.config),null,2) }}</pre></details><button class="danger" @click="remove">Supprimer le nœud</button></template></template><p v-else class="muted">Sélectionnez un nœud pour modifier ses propriétés.</p></aside>
      </div>
</template>
