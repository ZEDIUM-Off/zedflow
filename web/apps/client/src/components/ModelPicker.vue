...
const props = defineProps<{ selection?: ModelSelection; models: ModelEntry[]; disabled?: boolean; label?: string }>()
...
const budget = ref<number | undefined>(props.selection?.thinkingBudget)
watch(() => JSON.stringify(props.selection), () => {
  ...
  budget.value = value?.thinkingBudget
})
...
function commit() {
  if (provider.value && model.value.trim()) emit('change', { provider: provider.value, model: model.value.trim(), ...(effort.value ? { reasoningEffort: effort.value } : {}), ...(typeof budget.value === 'number' && Number.isFinite(budget.value) ? { thinkingBudget: budget.value } : {}) })
}
...
<template>
  <div class="model-picker" :aria-label="label || 'Réglages du modèle'">
    ...
    <label v-if="capability?.thinkingBudget">Budget de réflexion<input v-model.number="budget" type="number" min="0" step="1" placeholder="Défaut" :disabled="disabled" aria-label="Budget de réflexion" @change="commit"/></label>
  </div>
</template>
