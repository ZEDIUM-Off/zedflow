...
import type { NodeActivity, ToolActivity, Run, TimelineEntry } from '@zedflow/sdk'
...
type Row={id:string;entries:TimelineEntry[];passes:NodeActivity[];origins:NodeActivity[];tools:ToolActivity[]}
...
const rows=computed<Row[]>(()=>{
  ...
  const result=groups.map(group=>{
    ...
    const tools=group.flatMap(entry=>entry.kind==='tool'?[{...entry.activity,origin:entry.origin??null,timelineId:entry.id,timelineSeq:entry.seq}]:[])
