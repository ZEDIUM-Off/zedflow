...
import { modelSelectionSchema, jsonValueSchema, type JsonValue } from '@zedflow/sdk'
...
const props=defineProps<{run:Run|null;workspace?:Workspace;composition:Composition;flows:FlowFile[];models:ModelEntry[];nodes:ModelNode[];bindings:Record<string,HistoricalModelSelection>;context:RunContext|null|undefined;busy:boolean;composed?:boolean;initializing?:boolean;submit:(text:string,kind:'steering'|'followup',waitId?:string,nodePath?:string)=>Promise<void>}>()
const emit=defineEmits<{flow:[file:FlowFile];definition:[];composition:[];details:[tab:'models'|'context'];change:[path:string,selection:ModelSelection];answer:[value:JsonValue,waitId:string];remove:[id:string];command:[action:'abort'|'resume'];newSession:[]}>()
...
const availableSkills=computed(()=>{
  ...
  const pieces:AgentAttachments=requireNodeConfig((skillTarget.value?.context||skillTarget.value?.node)?.data.config ?? {}).attachments||{}
...
watch(()=>props.run?.wait?.id,()=>{waitingSelection.value=modelSelectionSchema.safeParse(props.bindings[waitPath.value]).data},{immediate:true})
watch(()=>JSON.stringify(props.bindings[waitPath.value]),()=>{waitingSelection.value=modelSelectionSchema.safeParse(props.bindings[waitPath.value]).data})
function choose(file:FlowFile){flowOpen.value=false;flowQuery.value='';emit('flow',file)}
function answer(value:unknown){const id=props.run?.wait?.id;if(id)emit('answer',jsonValueSchema.parse(value),id)}
