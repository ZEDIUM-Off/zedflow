...
let libraryIntent=0,libraryWorkspace:string|undefined
watch(()=>[props.workspaceId,referenceKey(config.value.contextLibraryRef),referenceHash(config.value.contextLibraryRef)],async()=>{const version=++libraryIntent,workspace=props.workspaceId,reference=config.value.contextLibraryRef,key=referenceKey(reference);if(libraryWorkspace!==workspace||libraryFile.value?.key!==key||libraryFile.value?.hash!==referenceHash(reference))libraryFile.value=undefined;libraryWorkspace=workspace;if(!key||!workspace)return;try{const file=await client.context.readLibrary(key,{workspaceId:workspace});if(version===libraryIntent)libraryFile.value=file}catch(cause){if(version===libraryIntent)error.value=String(cause instanceof Error?cause.message:cause)}},{immediate:true})
...
let typesIntent=0,typesWorkspace:string|undefined
watch(()=>[props.workspaceId,referenceKey(config.value.contextTypesRef),referenceHash(config.value.contextTypesRef)],async()=>{const request=++typesIntent,workspace=props.workspaceId,reference=config.value.contextTypesRef,key=referenceKey(reference);if(typesWorkspace!==workspace||typesFile.value?.key!==key||typesFile.value?.hash!==referenceHash(reference))typesFile.value=undefined;typesWorkspace=workspace;if(!key||!workspace)return;try{const file=await client.context.readTypes(key,{workspaceId:workspace});if(request===typesIntent)typesFile.value=file}catch(cause){if(request===typesIntent)error.value=cause instanceof Error?cause.message:String(cause)}},{immediate:true})
