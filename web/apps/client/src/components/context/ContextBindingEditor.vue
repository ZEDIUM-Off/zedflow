...
type Scope={kind:'runtime'}|{kind:'flow'|'bridge';id:string}
type ReaderBinding={kind:'reader';reader:string;input:{kind:'literal';value:JsonValue}|{kind:'state';field:string;pointer?:string}}
interface ReaderContract{id:string;version:string;input:ContextType;output:{kind:'fixed';dataType:ContextType}|{kind:'declaredJson'}}
type Binding={kind:'conversation';historyField:string;inputField:string}|{kind:'attachments';slot:'instructions'|'skills'|'files'}|ReaderBinding|{kind:'produced';producer:{branch:string;routeId?:string;input:ContextExpr;outputPointer?:string}}|{kind:'state';field:string;pointer?:string;encoding?:'adkMessages'}|{kind:'attachment';itemId:string;skillName?:string}|{kind:'entity';scope:Scope;alias:string;revision?:string}
const files=shallowRef<ContextFile[]>([]),loading=ref(false),validating=ref(false),error=ref(''),notice=ref(''),diagnostics=ref<ContextDiagnostic[]>([])
...
const key=computed(()=>typeof config.value.contextStrategy==='string'?config.value.contextStrategy:config.value.contextStrategy?.key||'')
...
const attachments=computed(()=>{
  const source=(config.value.attachments||{}) as AgentAttachments
  return [
...
const libraryMismatch=computed(()=>!!libraryFile.value&&!!config.value.contextLibraryRef?.hash&&libraryFile.value.hash!==config.value.contextLibraryRef.hash)
const hasHashMismatch=computed(()=>selected.value&&typeof config.value.contextStrategy==='object'&&config.value.contextStrategy.hash&&config.value.contextStrategy.hash!==selected.value.hash)
...
function bind(name:string,kind:string){
  ...
  config.value.contextBindings[name]=kind==='conversation'?{kind,historyField:'messages',inputField:'input'}:kind==='attachments'?{kind,slot:'instructions'}:kind==='reader'?{kind,reader:readers.value[0]?.id||'file.text',input:{kind:'literal',value:defaultContextValue(readers.value[0]?.input||{kind:'record',fields:{path:{kind:'text'}}})}}:kind==='state'?{kind,field:''}:kind==='attachment'?{kind,itemId:''}:kind==='produced'?{kind,producer:{branch:producerBranches.value[0]||'',input:textExpression()}}:{kind,scope:{kind:'runtime'},alias:''}
  notice.value='';error.value=''
}
function scope(binding:Binding,kind:string){if(binding.kind==='entity')binding.scope=kind==='runtime'?{kind}:{kind:kind as 'flow'|'bridge',id:''}}
function optional(binding:Binding,field:'pointer'|'encoding'|'skillName'|'revision',value:string){if(value)(binding as unknown as Record<string,string>)[field]=value;else delete (binding as unknown as Record<string,string>)[field]}
...
async function validate(){
  ...
  if(typesFile.value&&config.value.contextTypesRef?.hash&&typesFile.value.hash!==config.value.contextTypesRef.hash){error.value='Le catalogue de types a changé. Sélectionnez sa nouvelle version explicitement.';return}
  ...
  for(const [name,binding] of Object.entries(bindings.value)){
    if(binding.kind==='produced'&&!binding.producer.branch){error.value=`${name} : choisissez un branchement producteur exposé sur ce nœud.`;return}
  ...
  try{
    ...
    await client.context.validate({workspaceId:workspace,selection:{kind:'file',key:selected.value.key,hash:config.value.contextStrategy?.hash||selected.value.hash},types:types.value,library:libraryFile.value?.library||config.value.contextLibrary||{},resources:requirements.value,grantedCapabilities:grants})
...
let libraryIntent=0
watch(()=>[props.workspaceId,config.value.contextLibraryRef?.key,config.value.contextLibraryRef?.hash],async()=>{const version=++libraryIntent;libraryFile.value=undefined;const key=config.value.contextLibraryRef?.key;if(!key||!props.workspaceId)return;try{const file=await client.context.readLibrary(key,{workspaceId:props.workspaceId});if(version===libraryIntent)libraryFile.value=file}catch(cause){if(version===libraryIntent)error.value=String(cause instanceof Error?cause.message:cause)}},{immediate:true})
...
let typesIntent=0
watch(()=>[props.workspaceId,config.value.contextTypesRef?.key,config.value.contextTypesRef?.hash],async()=>{const request=++typesIntent;typesFile.value=undefined;const key=config.value.contextTypesRef?.key;if(!key||!props.workspaceId)return;try{const file=await client.context.readTypes(key,{workspaceId:props.workspaceId});if(request===typesIntent)typesFile.value=file}catch(cause){if(request===typesIntent)error.value=cause instanceof Error?cause.message:String(cause)}},{immediate:true})
