...
function choose(key:string){
  ...
  if(!file?.library){error.value='Choisissez une bibliothèque valide.';return}
  // Pin the visible snapshot before Save can capture the composition.
  emit('select',cloneContext(file))
}
watch(()=>[props.workspaceId,props.revision,props.active],(_,previous)=>{intent++;if(previous?.[0]!==props.workspaceId)files.value=[];if(props.active)void load()},{immediate:true})
