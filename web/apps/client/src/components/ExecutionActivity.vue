...
const props=defineProps<{activities:NodeActivity[];contextSnapshots?:import('@zedflow/sdk').ContextSnapshot[]}>()
...
function result(a:NodeActivity){if(!a.output)return undefined;if(a.output.toolResults?.length===1){const result=a.output.toolResults[0].result;return a.ui?.renderer==='table' && result!==null && typeof result==='object' && !Array.isArray(result) && Array.isArray(result.rows)?result.rows:result}if(a.output.toolResults?.length)return a.output.toolResults;const entries=Object.entries(a.output).filter(([k])=>!['messages','toolCalls','hasToolCalls'].includes(k));return entries.length===1?entries[0]![1]:a.output}
</script>
