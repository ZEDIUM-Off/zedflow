...
const props=withDefaults(defineProps<{snapshot?:import('@zedflow/sdk').ContextSnapshot;active?:boolean}>(),{active:true})
const {snapshot,entry,load}=useContextDetail(()=>props.snapshot,()=>props.active)
</script>
<template>
  <div v-if="snapshot" class="captured-context"><InferenceRaw v-if="snapshot.invocationId" :invocation="snapshot.invocationId" :revision="[snapshot.requestRef,snapshot.rawRef,snapshot.requestStatus].join('/')"/>
    ...
    <details v-for="resource in snapshot.resources||[]" :key="`${resource.name}:${resource.path}`" class="captured-resource"><summary>{{resource.name||resource.path||resource.id}}<span v-if="resource.truncated"> · partiel</span></summary><small>{{resource.kind}} · {{resource.hash}}</small><pre>{{resource.content}}</pre></details>
