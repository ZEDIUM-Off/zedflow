...
const runtimeGraphSummaryDefinition = z.object({ interactive: z.boolean().optional(), types: contextTypesSchema.optional(), entry: endpointSchema, instances: z.record(z.string(), z.object({ flow: z.string(), name: z.string(), hash: z.string(), interactive: z.boolean(), entries: z.record(z.string(), portContractSchema) }).catchall(jsonValueSchema)), inferences: z.record(z.string(), z.object({ instance: z.string(), node: z.string(), label: z.string(), config: jsonObjectSchema, contextProgramHash: z.string().nullish(), contextPath: z.string().nullish(), context: z.object({ node: z.string(), label: z.string(), config: jsonObjectSchema }).nullish() }).catchall(jsonValueSchema)), routes: z.record(z.string(), runtimeRouteSchema), aliases: z.record(z.string(), z.string()), dataBindings: z.record(z.string(), resolvedDataBindingSchema), bridges: z.array(z.string()), bridgeHashes: z.record(z.string(), z.string()).optional() }).catchall(jsonValueSchema);
