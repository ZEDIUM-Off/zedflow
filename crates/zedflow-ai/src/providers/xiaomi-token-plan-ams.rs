...
/// Creates the xiaomi-token-plan-ams provider from the static Rust model catalog.
pub fn xiaomi_token_plan_ams_provider() -> Result<Provider> {
    let provider = static_provider(
        XIAOMI_TOKEN_PLAN_AMS_PROVIDER_ID,
        XIAOMI_TOKEN_PLAN_AMS_PROVIDER_NAME,
        crate::providers::xiaomi_token_plan_ams_models::xiaomi_token_plan_ams_models(),
    );
    Ok(provider)
}
...
mod tests {
    ...
    fn builds_provider_from_static_catalog() {
        let provider = xiaomi_token_plan_ams_provider().expect("provider");
        assert_eq!(provider.id, XIAOMI_TOKEN_PLAN_AMS_PROVIDER_ID);
        assert_eq!(provider.name, XIAOMI_TOKEN_PLAN_AMS_PROVIDER_NAME);
