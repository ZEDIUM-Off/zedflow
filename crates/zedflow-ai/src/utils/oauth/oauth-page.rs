...
fn render_page(options: PageOptions<'_>) -> String {
    ...
    let details = options
        .details
        .map(|details| format!(r#"<div class="details">{}</div>"#, escape_html(details)))
        .unwrap_or_default();
...
/// Renders Pi's successful OAuth callback page.
#[must_use]
...
/// Renders Pi's failed OAuth callback page.
#[must_use]
...
mod tests {
    ...
    #[test]
    fn success_page_uses_pi_text_and_logo() {
    ...
    #[test]
    fn error_page_escapes_user_controlled_text() {
        let html = oauth_error_html(
            "Bad <message> & \"quote\"",
            Some("token='secret'\n<script>alert(1)</script>"),
        );

        assert!(html.contains("Bad &lt;message&gt; &amp; &quot;quote&quot;"));
