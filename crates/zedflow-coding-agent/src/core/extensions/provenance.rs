...
impl ExtensionSource {
    pub fn parse(value: &str) -> Result<Self, String> {
        if let Some(value) = value.strip_prefix("crate:") {
            ...
            if !is_component(name) || !is_exact_version(version) {
                return Err("crate source must name a crate and pin an exact version".into());
            }
        ...
        if let Some(value) = value.strip_prefix("github:") {
            ...
            if !is_component(owner)
                || !is_component(repo)
                || !is_commit(commit)
                || package.is_some_and(|value| !is_relative_path(value))
            {
...
mod tests {
    ...
    fn only_pinned_sources_are_accepted() {
        ...
        assert!(
            ExtensionSource::parse(
                "github:acme/demo@0123456789abcdef0123456789abcdef01234567#../other"
            )
            .is_err()
        );
    }
