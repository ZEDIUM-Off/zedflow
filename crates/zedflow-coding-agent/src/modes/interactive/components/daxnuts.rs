//! “Powered by daxnuts” animated transcript easter egg.

use zedflow_tui::{Component, visible_width};

use crate::modes_interactive_theme_theme::Theme;

const DAX_HEX: &str = "";
const WIDTH: usize = 32;
const HEIGHT: usize = 32;
const MAX_TICKS: usize = 25;
const RESET: &str = "\x1b[0m";

#[derive(Debug, Clone)]
pub struct DaxnutsComponent {
    image: Vec<String>,
    tick: usize,
    running: bool,
    accent: String,
    success: String,
    muted: String,
    dim: String,
    link: String,
}

impl DaxnutsComponent {
    #[must_use]
    pub fn new(theme: &Theme) -> Self {
        Self {
            image: build_image(),
            tick: 0,
            running: true,
            accent: theme.fg_ansi("accent").unwrap_or("").into(),
            success: theme.fg_ansi("success").unwrap_or("").into(),
            muted: theme.fg_ansi("muted").unwrap_or("").into(),
            dim: theme.fg_ansi("dim").unwrap_or("").into(),
            link: theme.fg_ansi("mdLink").unwrap_or("").into(),
        }
    }

    #[must_use]
    pub const fn tick_count(&self) -> usize {
        self.tick
    }

    #[must_use]
    pub const fn is_running(&self) -> bool {
        self.running
    }

    pub fn tick(&mut self) {
        if self.running {
            self.tick += 1;
            if self.tick >= MAX_TICKS {
                self.running = false;
            }
        }
    }

    pub fn dispose(&mut self) {
        self.running = false;
    }

    fn styled(&self, ansi: &str, text: &str) -> String {
        if ansi.is_empty() {
            text.into()
        } else {
            format!("{ansi}{text}\x1b[39m")
        }
    }
}

impl Component for DaxnutsComponent {
    fn render(&self, width: usize) -> Vec<String> {
        let center = |text: String| {
            format!(
                "{:padding$}{text}",
                "",
                padding = width.saturating_sub(visible_width(&text)) / 2
            )
        };
        let mut lines = vec![String::new()];
        let revealed = self
            .image
            .len()
            .min(self.tick * (self.image.len() + 3) / MAX_TICKS);
        for (row, image_line) in self.image.iter().enumerate() {
            lines.push(center(if row < revealed {
                image_line.clone()
            } else if row == revealed {
                format!("{}{}{RESET}", rgb(100, 200, 255, false), "▓".repeat(WIDTH))
            } else {
                " ".repeat(WIDTH)
            }));
        }
        lines.push(String::new());
        if self.tick > 15 {
            lines.push(center(
                self.styled(&self.accent, "Free Kimi K2.5 via OpenCode Zen"),
            ));
            lines.push(center(self.styled(&self.success, "\"Powered by daxnuts\"")));
            lines.push(center(self.styled(&self.muted, "— @thdxr")));
        } else {
            lines.extend([String::new(), String::new(), String::new()]);
        }
        lines.push(String::new());
        if self.tick > 17 {
            lines.push(center(self.styled(&self.dim, "Try OpenCode")));
            lines.push(center(
                self.styled(&self.link, "https://mistral.ai/news/mistral-vibe-2-0"),
            ));
        } else {
            lines.extend([String::new(), String::new()]);
        }
        lines.push(String::new());
        lines
    }
}

fn rgb(r: u8, g: u8, b: u8, background: bool) -> String {
    format!("\x1b[{};2;{r};{g};{b}m", if background { 48 } else { 38 })
}

fn build_image() -> Vec<String> {
    let bytes = DAX_HEX.as_bytes();
    (0..HEIGHT)
        .step_by(2)
        .map(|row| {
            let mut line = String::new();
            for x in 0..WIDTH {
                let top = pixel(bytes, row, x);
                let bottom = pixel(bytes, (row + 1).min(HEIGHT - 1), x);
                line.push_str(&rgb(bottom[0], bottom[1], bottom[2], false));
                line.push_str(&rgb(top[0], top[1], top[2], true));
                line.push('▄');
            }
            line.push_str(RESET);
            line
        })
        .collect()
}

fn pixel(bytes: &[u8], row: usize, x: usize) -> [u8; 3] {
    let offset = (row * WIDTH + x) * 6;
    [
        hex_byte(bytes, offset),
        hex_byte(bytes, offset + 2),
        hex_byte(bytes, offset + 4),
    ]
}

fn hex_byte(bytes: &[u8], offset: usize) -> u8 {
    (nibble(bytes[offset]) << 4) | nibble(bytes[offset + 1])
}

fn nibble(byte: u8) -> u8 {
    match byte {
        b'0'..=b'9' => byte - b'0',
        b'a'..=b'f' => byte - b'a' + 10,
        _ => unreachable!("frozen DAX_HEX is lowercase hexadecimal"),
    }
}
