...
fn pump_until_input(mode: &mut InteractiveMode, expected: &str) {
    let deadline = Instant::now() + Duration::from_secs(2);
    while Instant::now() < deadline {
        mode.pump_events(Duration::from_millis(20)).unwrap();
        if mode.get_user_input().as_deref() == Some(expected) {
            return;
        }
    }
    panic!("owner pump did not submit {expected:?}");
}
