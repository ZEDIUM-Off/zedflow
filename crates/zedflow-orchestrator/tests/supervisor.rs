...
#[tokio::test]
async fn unexpected_rpc_exit_clears_persisted_radius_pi_id() {
    let _env = ENV_LOCK.lock().unwrap_or_else(|error| error.into_inner());
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let address = listener.local_addr().unwrap();
    let server = thread::spawn(move || {
...
while IFS= read -r line; do
  ...
  case "$line" in
    *'"type":"get_state"'*)
      printf '{"type":"response","id":"%s","success":true,"command":"get_state","data":{"sessionId":"session-1"}}\n' "$id"
      [ "$prompt" -eq 1 ] && exit 0
      ;;
    *'"type":"prompt"'*)
      prompt=1
...
"#,
    ...
    supervisor
        ...
        .unwrap();
    for _ in 0..100 {
        let persisted = supervisor.get_instance(&instance.id).unwrap().unwrap();
        if persisted.status == InstanceStatus::Error && persisted.radius_pi_id.is_none() {
            break;
        }
        tokio::time::sleep(Duration::from_millis(10)).await;
    }
    ...
    assert_eq!(persisted.radius_pi_id, None);
    assert_eq!(
        server.join().unwrap(),
        [
            "/v1/machines/register",
...
#[tokio::test]
async fn stop_removes_persisted_instance_when_radius_disconnect_fails() {
    let _env = ENV_LOCK.lock().unwrap_or_else(|error| error.into_inner());
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let address = listener.local_addr().unwrap();
    let server = thread::spawn(move || {
    ...
    let instance = supervisor
        ...
        .await
        .unwrap();
    let error = supervisor.stop_instance(&instance.id).await.unwrap_err();
    assert!(error.to_string().contains("HTTP 500"));
    assert!(supervisor.get_instance(&instance.id).unwrap().is_none());
    assert_eq!(
        server.join().unwrap(),
        [
            "/v1/machines/register",
