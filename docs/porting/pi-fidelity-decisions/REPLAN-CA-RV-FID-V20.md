...
The `CA-RV-FID-V20-R2` reviewer is removed after deterministic evidence identified two repository gaps. `CA-RV-FID-V20-R2-REPAIR` attaches to its already-satisfied direct dependency and owns only `changelog.rs` and `fs-watch.rs`; `CA-RV-FID-V20-R2-V1` freshly validates the repair with the failed diff gate, formatting, focused compilation, and tests before `CA-NEXT-PORT-DAG-V18`.
