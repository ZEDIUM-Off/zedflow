...
`upgrade` is a one-shot reviewed recovery checkpoint, not a normal dispatch path. It requires the clean checked-out candidate HEAD to descend from integration, preserve the Pi gitlink, contain the approved recovery plan, leave a reachable DAG frontier, and name only failed/blocked units to supersede. It persists `ACCEPTING` before CAS so startup reconciliation can finish an interrupted upgrade without losing runtime history.
