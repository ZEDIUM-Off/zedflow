...
python3 tools/pi-port-swarm/controller.py scope-approve --request ID --approval-url https://api.github.com/repos/ZEDIUM-Off/zedflow/pulls/N/reviews/ID
python3 tools/pi-port-swarm/controller.py scope-wait --request ID [--timeout 3600]
```
...
DAG version 2 remains the legacy single-writer format. Version 3 requires `max_active_writers >= 2`, an exact `shared_files` list, and one `integration_lot` owner for every shared path. Independent producer writers execute concurrently from one immutable base without advancing integration. The integration lot applies every accepted producer diff, owns only exact declared shared paths, advances integration once by CAS, runs its own validation, and has exactly one direct validator followed by exactly one direct reviewer.
...
A running unit may request an exact scope extension against an evidence file, then wait with `scope-wait`; the request alone does not grant ownership. `scope-approve` accepts only an `APPROVED` review from this repository bound to a full commit SHA whose complete JSON body exactly names `request_id`, `unit`, `evidence_sha256`, and sorted `paths`. The immutable review identity and payload hash are retained, and candidate validation reads the final leased path set. An expired originating lease or conflicting path prevents the grant; declared shared files remain exclusive to `integration_lot`.
